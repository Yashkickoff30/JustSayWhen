...
<%
   ...
   String phno=request.getParameter("pmob");
   String ft=request.getParameter("tc");
   ...
   try{
		...
		PreparedStatement ps=conn.prepareStatement("insert into ppass values(?,?)");
		ps.setString(1,sid);
		ps.setString(2,pass);
		int i=ps.executeUpdate();
		ps=conn.prepareStatement("insert into photography1 values(?,?,?,?)");
		ps.setString(1,sid);
		ps.setString(2,sname);
		ps.setString(3,add);
		ps.setString(4,phno);
		 i=ps.executeUpdate();
		ps=conn.prepareStatement("insert into photography2 values(?,?,?)");
		ps.setString(1,sid);
		ps.setString(2,ft);
		ps.setString(3,budget);
		i=ps.executeUpdate();
				   String sql="Update ppass set ppass=ltrim(ppass)";
