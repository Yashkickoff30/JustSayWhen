...
<%
   ...
   String ft=request.getParameter("cpin");
   ...
   try{
		...
		PreparedStatement ps=conn.prepareStatement("insert into hpass values(?,?)");
		ps.setString(1,sid);
		ps.setString(2,pass);
		int i=ps.executeUpdate();
		ps=conn.prepareStatement("insert into hall1 values(?,?,?,?)");
		ps.setString(1,sid);
		ps.setString(2,sname);
		ps.setString(3,add);
		ps.setString(4,phno);
		 i=ps.executeUpdate();
		ps=conn.prepareStatement("insert into hall2 values(?,?,?,?)");
		ps.setString(1,sid);
		ps.setString(2,ft);
		ps.setString(3,guest);
		ps.setString(4,budget);
		i=ps.executeUpdate();
				   String sql="Update hpass set hpass=ltrim(hpass)";
