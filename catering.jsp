...
<%
   ...
   String nog=request.getParameter("gs");
   String avail=request.getParameter("av");
   ...
   try{
		...
		PreparedStatement ps=conn.prepareStatement("insert into cpass values(?,?)");
		ps.setString(1,sid);
		ps.setString(2,pass);
		int i=ps.executeUpdate();
		ps=conn.prepareStatement("insert into catering1 values(?,?,?,?)");
		ps.setString(1,sid);
		ps.setString(2,sname);
		ps.setString(3,add);
		ps.setString(4,phno);
		 i=ps.executeUpdate();
		ps=conn.prepareStatement("insert into catering2 values(?,?,?)");
		ps.setString(1,sid);
		ps.setString(2,ft);
		ps.setString(3,fp);
		i=ps.executeUpdate();
		ps=conn.prepareStatement("insert into catering3 values(?,?,?,?)");
		ps.setString(1,sid);
		ps.setString(2,nog);
		ps.setString(3,avail);
		ps.setString(4,budget);
		i=ps.executeUpdate();
		   String sql="Update cpass set cpass=ltrim(cpass)";
