...
<%
   ...
   String ft=request.getParameter("t");
   ...
   try{
		...
		PreparedStatement ps=conn.prepareStatement("insert into opass values(?,?)");
		ps.setString(1,sid);
		ps.setString(2,pass);
		int i=ps.executeUpdate();
		ps=conn.prepareStatement("insert into orchestra1 values(?,?,?,?)");
		ps.setString(1,sid);
		ps.setString(2,sname);
		ps.setString(3,add);
		ps.setString(4,phno);
		 i=ps.executeUpdate();
		ps=conn.prepareStatement("insert into orchestra2 values(?,?,?)");
		ps.setString(1,sid);
		ps.setString(2,ft);
		ps.setString(3,budget);
		i=ps.executeUpdate();
				   String sql="Update opass set opass=ltrim(opass)";
